...
public:
    ...
    Response* callback(Request* req, interface &manager) {
...
void Server::run(interface &manager) {
    ::listen(sc_, 10);
        ...
        try {
            ...
            for (; i < routes_.size(); i++) {
                if (routes_[i]->isMatch(req->getMethod(), req->getPath())) {
                    res = routes_[i]->handle(req, manager);
                    break;
                }
            }
            if (i == routes_.size() && notFoundHandler_) {
                res = notFoundHandler_->callback(req, manager);
            }
            delete req;
        }
...
Response* ShowFile::callback(Request* req, interface &manager) {
...
Response* TemplateHandler::callback(Request* req, interface &manager) {
