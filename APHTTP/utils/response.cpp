...
void Response::setCode (std::string_view message) {
    if (message == error_message::BAD_REQUEST) 
        code_ = static_cast<int>(Status::badRequest);
    else if (message == error_message::NOT_FOUND)
        code_  = static_cast<int>(Status::notFound);
    else if (message == error_message::PERMISSION_DENIED)
        code_ = static_cast<int>(Status::forbidden);
    else if (message == error_message::EMPTY)
        code_ = static_cast<int>(Status::seeOther);
    else
        code_ = static_cast<int>(Status::ok);    
}

void Response::setPhrase (std::string_view message) {
    phrase_ = std::string(message);
}
